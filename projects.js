...
async function createProject(projectName) {
  ...
  try {
    return octokit.projects.createForRepo(data);
  }
  catch (err) {
...
async function addColumnsToProject(columnName, projectId) {
  ...
  try {
    return await octokit.projects.createColumn(data);
  }
  catch (err) {
    console.log(err);
  };
...
async function reOrderColumns(columns) {
  ...
  for (const index in data) {
    await octokit.projects.moveColumn(data[index]);
  };
}
...
function addIssuesToColumn(columnId, issueIds) {
  issueIds.forEach((issueId) => {
    ...
    try {
      octokit.projects.createCard(data);
    }
    catch (err) {
      console.log(err);
    }
